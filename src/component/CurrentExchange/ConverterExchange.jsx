...
const ConverterExchange = (props) => {
    ...
    const rateItem = state.rateForElem.map(item => <li className={style.list__item}><div><p><span>1</span> {state.baseCurrency}</p></div><div><p><span>{item[1]}</span> {item[0]}</p></div></li>)
